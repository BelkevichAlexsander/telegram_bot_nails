...
async def command_start(message: types.Message):
    ...
    if message.from_user.id in ID:
        await sqlite_db.sql_add_user(
            state=(message.from_user.id, message.from_user.full_name, 'admin')
        )
        ...
        await sqlite_db.sql_add_user(
            state=(message.from_user.id, message.from_user.full_name, 'user')
        )
...
async def head_menu_handler(message: types.Message):
    ...
    """
    await message.delete()
    await message.answer("Выберите услугу для записи: ", reply_markup=entry_ikb.buttons_entry_menu)
...
async def price_from_main_menu(message: types.Message):
    ...
    """
    if message.from_user.id in ID:
        await message.answer(MESSAGES_CONTACTS['contacts'], reply_markup=keyboard_menu.main_menu_admin)
    else:
        await message.answer(MESSAGES_CONTACTS['contacts'], reply_markup=keyboard_menu.main_menu_user)
...
def registration_handler_start_menu(dp: Dispatcher):
    ...
    dp.register_message_handler(head_menu_handler, Text(equals=['Записаться'], ignore_case=True))
    dp.register_message_handler(price_from_main_menu, Text(equals=['Контакты'], ignore_case=True))
    dp.register_message_handler(admin_menu_handler, Text(equals=['Админ_панель'], ignore_case=True))
