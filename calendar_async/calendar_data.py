...
from message.month_message import MESSAGES_MONTH
...
class SimpleCalendar:
    async def start_calendar(
        ...
        inline_kb.insert(InlineKeyboardButton(
            "ГОД <<",
            callback_data=calendar_callback.new("ПРЕД-ГОД", year, month, 1)
        ))
        inline_kb.insert(InlineKeyboardButton(
            f'{MESSAGES_MONTH[month]} {str(year)}',
            callback_data=ignore_callback
        ))
        inline_kb.insert(InlineKeyboardButton(
            ">> ГОД",
            callback_data=calendar_callback.new("СЛЕД-ГОД", year, month, 1)
        ))
        ...
        for week in month_calendar:
            ...
            for day in week:
                if (day == 0):
                    ...
                    continue
                inline_kb.insert(InlineKeyboardButton(
                    str(day), callback_data=calendar_callback.new("ДЕНЬ", year, month, day)
                ))

        # Last row - Buttons
        inline_kb.row()
        inline_kb.insert(InlineKeyboardButton(
            "МЕСЯЦ <", callback_data=calendar_callback.new("ПРЕД-МЕСЯЦ", year, month, day)
        ))
        inline_kb.insert(InlineKeyboardButton(f"{MESSAGES_MONTH[month]}", callback_data=ignore_callback))
        inline_kb.insert(InlineKeyboardButton(
            "> МЕСЯЦ", callback_data=calendar_callback.new("СЛЕД-МЕСЯЦ", year, month, day)
        ))
    ...
    async def process_selection(self, query: CallbackQuery, data: CallbackData) -> tuple:
        ...
        if data['act'] == "ПРЕД-МЕСЯЦ":
            prev_date = temp_date - timedelta(days=1)
            await query.message.edit_reply_markup(await self.start_calendar(int(prev_date.year), int(prev_date.month)))
        # user navigates to next month, editing message with new calendar
        if data['act'] == "СЛЕД-МЕСЯЦ":
